...
<?php foreach ($order['products'] as $product): ?>
    <div class="product">
        <img   width="100" src="public/<?=$product['url']?>" alt="<?=$product['name']?>">
        <h3><?=$product['name']?></h3>
        <p><?=$product['price']?> &#8381;</p>
...
<?php endforeach;?>
<h2>Итого: </h2>
<p>Общее количество:<?=$order['total_count']?> шт</p>
<p>Общая стоимость: <?=$order['total_price']?> &#8381;</p>

<form action="order/send" method="post" style="display:flex; flex-direction:column"> 
 <div>
    ...
    <textarea name="adr" id="" cols="22" rows="5"></textarea></label>
    <br>
